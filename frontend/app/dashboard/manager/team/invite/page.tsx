...
import { PlusCircle, Edit, MoreHorizontal, Mail, Phone, Trophy, MapPin } from "lucide-react"
...
export default function TeamManagementPage() {
  ...
  // Fetch runners on page load
  useEffect(() => {
    const fetchRunners = async () => {
      try {
        const response = await axios.get(`/manager/runners?manager_id=${currentUser?.id}`)
  ...
  // Send invite
  const sendInvite = async (runnerId: number) => {
    try {
      await axios.post(`/manager/send/${runnerId}?manager_id=${currentUser?.id}`, {
  ...
  if (loading) return <div className="text-white p-4">Loading runners...</div>

  return (
    <div className="space-y-8">
      <h1 className="heading-font text-3xl font-bold text-[#EAEAE8] mb-4">Team Roster</h1>
            ...
            <thead className="bg-[#868684]/10 border-b border-[#868684]/20">
              <tr>
                <th className="text-left p-4">
                  <input
                    type="checkbox"
                    checked={selectedMembers.length === teamRoster.length && teamRoster.length > 0}
                    onChange={() => setSelectedMembers(selectedMembers.length === teamRoster.length ? [] : teamRoster.map(m => m.id))}
              ...
              {teamRoster.map((member, index) => (
                <tr
                  ...
                  className={`border-b border-[#868684]/10 hover:bg-[#868684]/5 transition-colors ${
                    loading ? `opacity-0 translate-y-4` : `opacity-100 translate-y-0 delay-${index * 50}`
                  }`}
                >
                  <td className="p-4">
                    <input
                  ...
                  <td className="p-4">
                    <div>
                      <h3 className="font-bold text-white mb-1">{member.first_name}</h3>
                      <div className="text-sm text-[#868684] mb-1">{member.role || "Runner"}</div>
                  ...
                  <td className="p-4">
                    <div>
                      ...
                      <div className="text-sm text-[#868684] mb-2">Best: {member.personal_best_5k || "-"}</div>
                      <div className="flex items-center space-x-2 text-xs text-[#868684]">
                        <Trophy className="w-3 h-3" />
                        <span>{member.racesCompleted || 0} races</span>
                      </div>
                    </div>
                  </td>
                  {/* <td className="p-4">
                    <span
                      className={`px-3 py-1 text-xs font-bold rounded-full border ${getStatusColor(member.status || "pending")}`}
                    >
